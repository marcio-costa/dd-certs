...
**Live Tail** — Real-time streaming view of all ingested logs (indexed or not), after pipeline processing. No historical lookback; output is uniformly sampled when the query matches very high volume.
...
**Restriction query** — RBAC query attached to a **role** that scopes the *Logs Read Data* permission to matching logs (e.g., `team:acme`). One restriction query per role; managed via the Data Access page or Roles API — not a per-index setting.

**Retention** — Configurable per-index search window (3, 7, 15, 30, 45, 60, 90 days; longer on some plans).
