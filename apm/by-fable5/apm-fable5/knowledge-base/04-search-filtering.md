...
|---|---|---|
| Time range | **Real-time stream** as logs are ingested — no historical lookback | **Index retention** (typically 15 / 30 days+) |
| What you see | **All ingested logs**, even those *not* indexed (sampled uniformly if the query matches a very high volume) | **Only indexed logs** |
...
- **Live Tail = real-time stream of every ingested log (indexed or not).** Explorer = retained / indexed logs. Live Tail has no historical lookback and samples output when volume is very high.
