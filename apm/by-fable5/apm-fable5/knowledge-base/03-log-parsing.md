...
### Preprocessing precedence (corner case the exam loves)

JSON **preprocessing runs before any pipeline** and maps JSON keys onto the reserved attributes using configurable attribute lists (*Logs → Configuration → Pipelines → Preprocessing for JSON logs*). Defaults include: `host`/`hostname`/`syslog.hostname` → `host`, `service`/`syslog.appname` → `service`, `level`/`severity`/`syslog.severity` → `status`, `timestamp`/`date`/`syslog.timestamp` → date, `message`/`msg`/`log` → `message`.

**If a JSON log contains one of these keys, its value overrides what the Agent set at collection time.** Example: the Agent config says `service: checkout`, but the JSON body contains `"service": "payments"` → the log lands with `service:payments`. In Kubernetes this bites hard with `host`: a JSON `host` field overrides the Agent hostname and the log loses its host-level tags — Datadog recommends clearing such attributes.
