...
## 7.7 UI-based diagnostics

Three places in the Datadog UI help you debug processing and indexing:

- **Pipeline Scanner** (*Logs → Pipelines → Pipeline Scanner*, or from a log's side panel) — paste/select a live log and see **exactly which pipelines and processors touched it**, in order. This is the fastest way to answer "why isn't my log parsed the way I expect?"
- ***Logs → Pipelines* list** — shows per-pipeline filters and lets you test **sample logs** inside each processor editor before saving.
- **Usage / estimated usage views** (*Logs → Configuration → Indexes* volume graphs and the Log Management usage pages) — sanity-check how much each index is ingesting/excluding, and confirm exclusion filters behave as intended (Datadog also emits `datadog_index` events like "daily quota reached" you can search with `source:datadog`).
