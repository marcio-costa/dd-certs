...
| **Restriction query** (RBAC) | Attached to a **role** (Data Access page / Roles API) — *not* an index setting | Scopes the *Logs Read Data* permission so the role only sees logs matching a query (e.g., `team:acme`); one restriction query per role |
...
3. **Retention** — how long indexed logs are searchable (3, 7, 15, 30, 45, 60, 90 days; longer options like 180/365 exist on some plans).
...
- To see logs that were **excluded from indexing**, use **Archives + Rehydrate** (historical) or **Live Tail** (real time, as they stream in).
