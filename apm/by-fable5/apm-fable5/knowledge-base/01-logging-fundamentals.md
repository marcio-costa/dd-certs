...
- Viewed in **Live Tail** in real time (streaming view of all ingested logs)
...
[Processing pipelines]   ◀─── grok, remappers, lookup, category
   │
   ├─► Live Tail (real-time stream — sees all ingested logs)
...
|---|---|
| Searchable retention | **Index retention** (3, 7, 15, 30, 45, 60, 90 days — longer options such as 180/365 exist on some plans) |
...
| Restrict who can see what | **RBAC** — **restriction queries** attached to roles (scope the *Logs Read Data* permission to a log query, e.g. `team:acme`) |
...
- Datadog supports eight reserved log severities (`emergency`…`debug`), plus the special `OK` status.
