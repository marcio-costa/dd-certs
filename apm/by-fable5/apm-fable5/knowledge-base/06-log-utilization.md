...
> Log monitors run against **indexed** logs. If your logs are excluded from the index they won't drive a log monitor — generate a metric from them instead and put a **metric monitor** on it.
...
### Alerting on non-indexed logs (corner case)

There is **no monitor type that watches Live Tail**. If you must alert on logs you've excluded from indexing, the supported pattern is: **Logs to Metrics rule → metric monitor**. This is a classic exam scenario — "we excluded debug logs but still need an alert on their volume."
...
```
        ...
        │  (real-time) │   │ (cheap, S3)  │         │  (volume/value)  │
        └──────────────┘   └──────────────┘         └──────────────────┘
                                    │
                                    ▼
                       ┌────────────────────────┐
...
- **Generated metrics** count toward your custom metrics quota (log-based metrics are retained **15 months** at 10-second granularity).

> **Awareness — Flex Logs:** Datadog also offers a **Flex Logs** storage tier for high-volume logs that need long, occasionally-queried retention without full indexing cost. Know that it exists and sits between standard indexing and archives; deep configuration detail is beyond the fundamentals exam.
...
- **Log monitors run on indexed logs.** To alert on excluded logs: Logs to Metrics → metric monitor.
