...
**Q14.** A team's Agent config sets `service: checkout`, but the JSON log body also contains `"service": "payments"`. After ingestion, the log's `service` is:

A) `checkout` — the Agent's collection-time value always wins.
B) `payments` — JSON preprocessing maps the reserved `service` key and overrides the Agent-set value.
C) Both values, concatenated.
D) The log is rejected as ambiguous.
...
14. **B** — **Preprocessing for JSON logs** runs before pipelines and maps reserved JSON keys (`service`, `host`, `status` via `level`/`severity`, `timestamp`, `message`) onto the log — overriding the Agent-set values. Same mechanism makes a JSON `host` field override the Agent hostname in Kubernetes (a documented gotcha). To keep the Agent value, remove/rename the JSON key or edit the preprocessing attribute list.
