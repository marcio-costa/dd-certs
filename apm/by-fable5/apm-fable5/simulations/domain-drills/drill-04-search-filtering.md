...
**Q7.** Which THREE statements about Saved Views are TRUE? (Select THREE)
...
**Q8.** Live Tail vs. Explorer — which THREE statements are TRUE? (Select THREE)

A) Live Tail streams logs in near real time as they are ingested, including non-indexed logs.
B) Live Tail does not support visualization tabs (Top List, Pie, etc.).
C) Live Tail shows only indexed logs.
D) Live Tail lets you scroll back through the full index retention window.
...
**Q15.** A team wants to share a Saved View URL with a teammate. The teammate sees no results because their role's log visibility is scoped to a query that excludes those logs. Which Datadog feature enforces this?

A) Restriction queries (RBAC) attached to the teammate's role.
B) Pipeline filters.
C) Multi-Alert.
D) Generate Metrics.
...
7. **A, C, D** — Saved Views persist query/time/viz/facets, keep relative ranges live, and their URL accepts a `?query=` override. (B) API keys are never stored in views; (E) no fixed auto-refresh.
8. **A, B, E** — Real-time stream of all ingested logs, list-only view, same query syntax. (C) and (D) are false — no index dependency, no historical lookback.
...
15. **A** — Restriction queries are role-level RBAC: they scope the *Logs Read Data* permission to a log query (one restriction query per role).
