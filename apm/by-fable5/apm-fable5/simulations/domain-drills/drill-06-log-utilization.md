...
**Q10.** Which THREE statements about Log Forwarding are TRUE? (Select THREE)
...
**Q12.** A user checks a monitor's underlying query in the Explorer and sees fewer logs than the monitor counts. Likely cause:

A) The user's role has a restriction query (RBAC) that hides part of the logs from *them* — monitors evaluate the full indexed data.
B) Live Tail interfered.
C) The monitor's threshold rejected them.
D) `index:*` is invalid syntax.

**Q13.** Which THREE are valid in a log monitor's notification body? (Select THREE)

A) `@email` recipients in the message.
B) `@slack-channel-name` for Slack.
C) Inline `{{value}}` template variable.
D) Nested SQL queries.
E) Live JavaScript snippets executed on render.
...
10. **A, B, E** — Multiple destinations, real-time copy, and no effect on indexing decisions. (C) no Agent needed — forwarding is server-side; (D) forwarding complements archives, it doesn't replace them.
11. **B** — Logs to Metrics works on excluded logs.
12. **A** — Restriction queries are role-level RBAC on read visibility; the monitor itself sees all indexed logs, so a restricted user can see a smaller count than the monitor reports.
13. **A, B, C** — Email handles, Slack channel handles, and template variables are all supported in monitor messages. SQL and JavaScript are not.
