...
**Q11.** In Kubernetes with `container_collect_all` enabled and **no** pod annotation or Unified Service Tagging label, the default `source` and `service` applied to a container's logs are:

A) The container's **short image name** (e.g., `nginx`) for both.
B) The literal string `unknown` for both.
C) The pod name for `service` and the namespace for `source`.
D) Empty — Datadog rejects logs without a service.
...
11. **A** — With no annotation/label override, autodiscovery falls back to the container's **short image name** for both `source` and `service` (which is why `nginx` containers get the NGINX pipeline automatically). Override per container with `ad.datadoghq.com/<container>.logs` or UST labels. For host file-tailing, always set `service:`/`source:` explicitly — logs without them are hard to route and correlate.
