...
**Q1.** A log has both a JSON `service` field and an Agent-set `service:` in `conf.yaml`. Which value wins on the log envelope?

A) The Agent-set value (collection-time) · B) The JSON value — preprocessing for JSON logs maps reserved keys over Agent-set values · C) Random · D) They are concatenated
...
1. B 2. A 3. A, C 4. A 5. A 6. B 7. B 8. B 9. A 10. B

> Q1 note: *Preprocessing for JSON logs* runs before pipelines and maps reserved JSON keys (`service`, `host`, `level`/`severity` → `status`, `timestamp`, `message`/`msg`) over the Agent-set envelope values. Same reason a JSON `host` field can break host tags in Kubernetes.
