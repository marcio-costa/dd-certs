...
**Q8.** Which THREE are valid `type:` values in a `logs:` config? (Select THREE)
...
8. A, B, E — `file`, `tcp`, `windows_event` (also valid: `udp`, `journald`, `docker`). `journal` is misspelled; `http` isn't an Agent type.
