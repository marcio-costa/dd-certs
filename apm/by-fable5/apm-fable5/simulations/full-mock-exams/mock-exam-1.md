...
**Q16.** Which THREE are valid `type:` values inside a `logs:` entry? (Select THREE)
...
**Q36.** Which best describes Live Tail's time scope?

A) It replays the last 24 hours of ingested logs.
B) It streams logs in real time as they are ingested, with no historical lookback.
C) It shows the last hour of indexed logs.
D) Its window equals index retention.
...
| 11 | A, B, D | Preprocessing for JSON logs maps `host`/`hostname`, `level`/`severity` → `status`, and `timestamp`/`date` (also `service` and `message`/`msg`). `version` (C) and `env` (E) are Unified Service *tags*, not preprocessed reserved attributes. |
...
| 36 | B | Live Tail = real-time stream of ingested logs (post-pipeline), sampled at very high volume; no lookback. |
...
| 65 | A, B, E | Splunk HEC, ES/OpenSearch, and Microsoft Sentinel (a generic HTTP endpoint is also valid but not listed). Redshift, DynamoDB, MongoDB Atlas are not forwarding destinations. |
