...
D) Automatically delete the matched log from archives.
...
**Q57.** Which TWO statements about restriction queries are TRUE? (Select TWO)

A) They scope a role's *Logs Read Data* permission so users only see logs matching a query (e.g., `team:acme`).
B) They drop logs that don't match.
C) They are part of Datadog's RBAC for logs and are attached to roles (one per role).
D) They override exclusion filters.
E) They are configured per index under *Logs → Configuration → Indexes*.
...
**Q60.** Which THREE statements about Logs to Metrics are TRUE? (Select THREE)

A) Generated metrics are retained for 15 months at 10-second granularity.
...
**Q67.** Which THREE are valid forwarding destinations? (Select THREE)
...
| 19 | A, B | SDS actions are **Redact**, **Partially Redact**, and **Hash** (irreversible). Rules also tag matched logs, but SDS never encrypts with customer KMS, deletes from archives, or drops logs. |
...
| 57 | A, C | Restriction queries are role-level RBAC (Data Access page / Roles API). They control read visibility only — they never drop logs (B), don't interact with exclusion filters (D), and are not an index setting (E). |
| 58 | A | Threshold + Multi-Alert grouped by service. |
| 59 | A | `index:*` queries every index. |
| 60 | A, B, D | Log-based metrics are computed from the full ingest stream (incl. excluded logs), retained 15 months at 10s granularity, with up to 10 group-by dimensions. (C) archives keep the logs; (E) no index needed — that's the whole point. |
...
| 67 | A, B, E | Sentinel, ES/OpenSearch, generic HTTP (Splunk HEC also valid). DynamoDB and MongoDB Atlas are not. |
