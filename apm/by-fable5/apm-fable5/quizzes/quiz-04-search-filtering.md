...
### 2. Which best describes the time scope of Live Tail?
...
C) It shows exactly the last hour of indexed logs.
D) Its window equals the index retention.
...
2. **B** — Live Tail is a real-time stream of all ingested logs (post-pipeline). There is no historical lookback; to look backward you need indexed logs (Explorer) or rehydration. At very high volume the stream is uniformly sampled.
