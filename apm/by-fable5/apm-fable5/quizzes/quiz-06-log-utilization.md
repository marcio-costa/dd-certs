...
8. **B, C, F** — Elasticsearch/OpenSearch, generic HTTP webhook, and Microsoft Sentinel (Splunk HEC is also a valid destination, just not listed here). QuickSight (A), DynamoDB (D), and MongoDB Atlas (E) are not forwarding destinations.
