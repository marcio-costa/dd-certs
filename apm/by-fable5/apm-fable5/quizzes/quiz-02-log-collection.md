...
### 5. Which THREE of the following are valid `type:` values in an Agent `logs:` configuration entry? (Select THREE)
...
5. **A, B, E** — `file`, `tcp`, `udp`, `journald`, `windows_event`, `docker` are the valid types. `journal` (C) is misspelled; `http` (D) isn't an Agent listener type — direct HTTP submission is via the intake API, not the Agent.
